...
def train_val_split(labelme_dataset_dir, ouput_dataset_dir, image_name, train_val_ratio=0.2):
    ...
    for i in range(len(train_idxs)):
        f_name = os.path.splitext(json_files[train_idxs[i]])[0]
        if path.exists(labelme_dataset_dir + "/" + f_name + ".JPG"):
            shutil.copy2(labelme_dataset_dir + "/" + f_name + ".JPG",
                         train_image_folder)  # move train txt
        if path.exists(labelme_dataset_dir + "/" + f_name + ".txt"):
            shutil.copy2(labelme_dataset_dir + "/" + f_name + ".txt",
                         train_label_folder)  # move train image
        if path.exists(labelme_dataset_dir + "/" + f_name + ".json"):
            shutil.copy2(labelme_dataset_dir + "/" + f_name + ".json",
                         train_json_folder)  # move train json
    for i in range(len(val_idxs)):
        f_name = os.path.splitext(json_files[val_idxs[i]])[0]
        if path.exists(labelme_dataset_dir + "/" + f_name + ".JPG"):
            shutil.copy2(labelme_dataset_dir + "/" + f_name + ".JPG",
                         val_image_folder)  # move val txt
        if path.exists(labelme_dataset_dir + "/" + f_name + ".txt"):
            shutil.copy2(labelme_dataset_dir + "/" + f_name + ".txt",
                         val_label_folder)  # move val image
        if path.exists(labelme_dataset_dir + "/" + f_name + ".json"):
            shutil.copy2(labelme_dataset_dir + "/" + f_name + ".json",
                         val_json_folder)  # move val json
...
if __name__ == '__main__':
    ...
    train_val_split(args.labelme_dataset_dir, args.ouput_dataset_dir, args.image_name,
